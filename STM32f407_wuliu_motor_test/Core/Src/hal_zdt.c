...
zdt_ret_t zdt_origin_set(uint8_t addr, bool save)
{
  uint8_t cmd[5];
...
zdt_ret_t zdt_home_trigger(uint8_t addr, uint8_t mode, bool sync)
{
  uint8_t cmd[5];
...
zdt_ret_t zdt_home_abort(uint8_t addr)
{
  uint8_t cmd[4];
...
zdt_ret_t zdt_home_params_read(uint8_t addr)
{
  uint8_t cmd[3];
  ...
  cmd[1] = 0x22;
  cmd[2] = 0x6B;
  return zdt_tx(cmd, 3U);
}

zdt_ret_t zdt_home_params_write(uint8_t addr, bool save, uint8_t mode, uint8_t dir, uint16_t vel,
                                uint32_t timeout_ms, uint16_t sensorless_vel,
                                uint16_t sensorless_current_ma, uint16_t sensorless_time_ms,
                                bool auto_home)
{
  uint8_t cmd[20];
  ...
  cmd[6] = (uint8_t)(vel >> 8);
  cmd[7] = (uint8_t)(vel >> 0);
  cmd[8] = (uint8_t)(timeout_ms >> 24);
  cmd[9] = (uint8_t)(timeout_ms >> 16);
  cmd[10] = (uint8_t)(timeout_ms >> 8);
  cmd[11] = (uint8_t)(timeout_ms >> 0);
  cmd[12] = (uint8_t)(sensorless_vel >> 8);
  cmd[13] = (uint8_t)(sensorless_vel >> 0);
  cmd[14] = (uint8_t)(sensorless_current_ma >> 8);
  cmd[15] = (uint8_t)(sensorless_current_ma >> 0);
  cmd[16] = (uint8_t)(sensorless_time_ms >> 8);
  cmd[17] = (uint8_t)(sensorless_time_ms >> 0);
  cmd[18] = auto_home ? 1U : 0U;
  cmd[19] = 0x6B;
  return zdt_tx(cmd, 20U);
}

zdt_ret_t zdt_clear_pos(uint8_t addr)
{
  uint8_t cmd[4];
...
zdt_ret_t zdt_clear_stall(uint8_t addr)
{
  uint8_t cmd[4];
