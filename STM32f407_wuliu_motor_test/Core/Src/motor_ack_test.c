...
#define ZDT_HOME_SHENGJIANG_CURRENT_MA 600U
#define ZDT_HOME_SHENGJIANG_TIME_MS 40U
...
static void servo_defaults(void)
{
                                    ...
                                    uint32_t timeout_ms, uint8_t *status)
{
  ...
  (void)zdt_en(SHENGJIANG_ADDR, true, false);
  delay_ms1(50U);
  (void)zdt_home_cfg_sensorless(SHENGJIANG_ADDR, dir, current_ma, time_ms);
  delay_ms1(20U);
  ...
  delay_ms1(20U);
  shengjiang_set_current_pos(0.0f);
  return idle && ((*status & ZDT_HOME_STATUS_FAIL) == 0U);
}
...
static void shengjiang_homezero_default(const char *tag)
{
  ...
  txs("# ");
  txs(tag);
  txs(" SJ HOMEZERO START mode=2 dir=1 current=");
  txu(ZDT_HOME_SHENGJIANG_CURRENT_MA);
  txs(" time=");
  txu(ZDT_HOME_SHENGJIANG_TIME_MS);
  txs(" timeout=");
  txu(ZDT_HOME_TIMEOUT_MS);
  txs("\r\n");

  ok = shengjiang_homezero_run(1U,
...
static zdt_ret_t zdt_home_cfg_sensorless(uint8_t addr, uint8_t dir, uint16_t current_ma, uint16_t time_ms)
{
  return zdt_home_params_write(addr, false, 2U, dir, ZDT_HOME_VEL, ZDT_HOME_TIMEOUT_MS,
...
static void handle_shengjiang(char *params)
{
  ...
  if (starts_with(cursor, "HOMEZERO"))
  {
    ...
    int32_t timeout_ms = ZDT_HOME_TIMEOUT_MS;
    uint8_t status = 0U;
    bool ok;
...
static void act_fang(uint8_t code)
{
  ...
  if (code == 1U)
  {
    act_pose(4.0f, 25, ACT_YT_RIGHT_RING_DEG);
    act_pose(4.0f, 132, ACT_YT_RIGHT_RING_DEG);
  }
  else if (code == 2U)
  {
    act_pose(-20.0f, 25, ACT_YT_PLACE_RACK_DEG);
    act_pose(-20.0f, 25, ACT_YT_CENTER_RING_DEG);
    act_pose(-50.0f, 132, ACT_YT_CENTER_RING_DEG);
  }
  else
  {
    act_pose(8.8f, 25, ACT_YT_LEFT_RING_DEG);
    act_pose(8.8f, 132, ACT_YT_LEFT_RING_DEG);
  }
  set_zhuashou_kai();
}
...
static void act_fang_maduo(uint8_t code)
{
  ...
  if (code == 1U)
  {
    act_pose(3.0f, 25, ACT_YT_RIGHT_RING_DEG);
    act_pose(3.0f, 60, ACT_YT_RIGHT_RING_DEG);
  }
  else if (code == 2U)
  {
    act_pose(ACT_X_RACK, 25, ACT_YT_PLACE_RACK_DEG);
    act_pose(-120.0f, 25, ACT_YT_CENTER_RING_DEG);
    act_pose(-50.0f, 60, ACT_YT_CENTER_RING_DEG);
  }
  else
  {
    act_pose(ACT_X_RACK, 25, ACT_YT_PLACE_RACK_DEG);
    act_pose(-120.0f, 25, ACT_YT_LEFT_RING_DEG);
    act_pose(3.0f, 60, ACT_YT_LEFT_RING_DEG);
  }
  set_zhuashou_kai();
}
...
static void act_na(uint8_t code)
{
  if (code == 1U)
  {
    act_pose(3.0f, 130, ACT_YT_RIGHT_RING_DEG);
    set_zhuashou_he();
    act_pose(-50.0f, 50, ACT_YT_RIGHT_RING_DEG);
  }
  else if (code == 2U)
  {
    act_pose(-10.0f, 130, ACT_YT_CENTER_RING_DEG);
    act_pose(-50.0f, 130, ACT_YT_CENTER_RING_DEG);
    set_zhuashou_he();
    act_pose(-50.0f, 50, ACT_YT_CENTER_RING_DEG);
  }
  else
  {
    act_pose(3.0f, 110, ACT_YT_LEFT_RING_DEG);
    act_pose(3.0f, 130, ACT_YT_LEFT_RING_DEG);
    set_zhuashou_he();
    act_pose(8.0f, 25, ACT_YT_LEFT_RING_DEG);
  }
  act_pose(ACT_X_RACK, ACT_H_TRANSFER, ACT_YT_PLACE_RACK_DEG);
  act_rack_ready();
  set_zhuashou_kai();
...
static void act_yuantai_na(uint8_t code)
{
  ...
  if (code == 1U)
  {
    act_pose(-100.0f, 50, ACT_YT_CENTER_RING_DEG);
    set_zhuashou_he();
  }
  else if (code == 2U)
  {
    act_pose(63.0f, 48, ACT_YT_LEFT_RING_DEG);
    set_zhuashou_he();
    act_pose(20.0f, 25, ACT_YT_LEFT_RING_DEG);
    act_pose(-20.0f, 25, ACT_YT_PLACE_RACK_DEG);
  }
  else
  {
    act_pose(63.0f, 48, ACT_YT_RIGHT_RING_DEG);
    set_zhuashou_he();
  }
  act_pose(ACT_X_RACK, ACT_H_TRANSFER, ACT_YT_PLACE_RACK_DEG);
  act_rack_ready();
...
static void act_yuantai_na1(uint8_t code)
{
  ...
  if (code == 1U)
  {
    act_pose(40.0f, 0, ACT_YT_CENTER_RING_DEG);
  }
  else if (code == 2U)
  {
    act_pose(63.0f, 18, ACT_YT_LEFT_RING_DEG);
  }
  else
  {
    act_pose(63.0f, 18, ACT_YT_RIGHT_RING_DEG);
  }
}
...
void motor_ack_test_init(void)
{
  ...
  shengjiang_homezero_default("BOOT");
  pingtui_set_current_pos(0.0f);
  (void)zdt_clear_pos(PINGTUI_ADDR);
  arm_rx();
  emit_status();
